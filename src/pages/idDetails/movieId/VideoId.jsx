...
const VideoId = () => {
  ...
  const url = baseUrl + `/movie/${id}?language=en-US&api_key=` + API_KEY;
  const link =
    baseUrl + `/movie/${id}/similar?language=en-US&api_key=` + API_KEY;
  ...
  return (
    <div className="container md:mt-6">
      ...
      <div className="bg-black w-full h-[280px] m-auto md:rounded-lg mb-8 backdrop-ratio">
        <img
          src={`${w500Img}${movieId.backdrop_path}`}
          alt=""
      ...
      <div className="flex_btw item-center mb-4 ">
        <div className="w-[160px] h-[230px] rounded-lg bg-black mr-6">
          <img
            src={`${w500Img}${movieId.poster_path}`}
            alt=""
        ...
        <div className="flex-1">
          ...
          <h2 className="text-[30px]">{movieId.title}</h2>

          <p className="w-[92%] mb-5 text-[15px]">{movieId.overview}</p>
            ...
            <div className="flex justify-start items-start mb-1">
              ...
              <p className="text-[14px] ml-[6px] flex-1">
                <span className="text-[15px] font-semibold">
                  {Math.round(`${movieId.vote_average}` * 10) / 10}
                </span>
                /10 <span className="ml-2">({movieId.vote_count})</span>
              </p>
            </div>
